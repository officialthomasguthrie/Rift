//! The Appearance page: the wallpaper, dark or light, the accent, the gaps between windows, the
//! corner radius of a window and the terminal greeting. Every change is written to the owner's
//! files and handed to the apps, the compositor and the shell at once.

use iced::widget::{button, column, container, row, text};
use iced::{Border, Center, Element, Fill, Length, Theme};
use librift::appearance::{Accent, GAPS_MOST, RADIUS_MOST, Theme as Mode};
...
pub fn view(state: &Settings, look: Colors) -> Element<'_, Message> {
    let later = note(
        look,
        "Interface text size, the terminal colour scheme and the boot style come with the rest of \
         Settings.",
    );
    let mut page = column![
        themes(state, look),
