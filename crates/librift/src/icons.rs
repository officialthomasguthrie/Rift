...
//! the Adwaita theme and then hicolor. A status icon is a symbolic drawing painted in one colour,
//! so those directories come first; an app icon is the app's own drawing in its own colours, so
//! that lookup starts at the app directories instead. The shell and Settings draw what is found
//! here, each with its own toolkit.
...
mod tests {
    ...
    fn temporary(name: &str) -> PathBuf {
        let root =
            std::env::temp_dir().join(format!("librift-icons-{name}-{}", std::process::id()));
