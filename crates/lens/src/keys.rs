...
pub fn volume(word: &str) -> Result<Level, String> {
    match word {
        "up" => status::step_volume(true)?,
        "down" => status::step_volume(false)?,
        "mute" => status::toggle_mute()?,
        _ => return Err(format!("Try {USAGE}.")),
    }
    let volume = status::volume().ok_or("wpctl reads no volume for the default sink.")?;
    Ok(Level::Volume {
