...
def check_apps(width, height, rgb, apps, colors=DARK_COLORS):
    ...
    is not the desktop's and not the gray of the window's content lower down, with something drawn in
    the right end of it, where the close button is. Returns (ok, lines to print)."""
    ...
    for x in range(width):
        ...
        gap.append(all(near(px, colors.desktop, 3) or near(px, colors.accent, 40) for px in pixels))
    windows = []
    x = 0
    while x < width:
        if gap[x]:
    ...
    for app, (left, right) in zip(apps, windows):
        ...
        for y in range(top_of_area, min(bottom_of_area, top_of_area + round(80 * scale))):
            ...
            if sum(1 for px in drawn if not near(px, colors.desktop, 3) and not near(px, colors.accent, 40)) >= 4:
                top = y
                break
        if top < 0:
        ...
        content, _ = most_common(rgb, width, left + inset, right - inset, top + round(110 * scale),
                                 top + round(150 * scale))
        ...
        checks += [
            (f"{app}'s title bar is one gray of the theme along the top of its window",
             neutral and shade and share >= 0.4 and not near(fill, colors.desktop, 3),
             f"from x {left} to {right}, top {top}: #{bytes(fill).hex()} over {share:.0%} of its first rows"),
            (f"{app}'s title bar is not the window's content", not near(fill, content, 6),
             f"#{bytes(fill).hex()} over the content's #{bytes(content).hex()}"),
...
def main():
    ...
    if args.desktop:
        ...
        def look(what, png, seconds, console=False, lock=None, apps=None, colors=DARK_COLORS, must=True,
                 journals=(), **shape):
            ...
            if not good:
                ...
                if not must:
                    print(f"\nboot-test: {what} is not on screen, see {png}; going on", flush=True)
                    return
        ...
        if args.lens:
            ...
            def unless(passed, why):
                if not passed:
                    if strict:
                        fail(why)
                    print(f"\nboot-test: {why}; going on", flush=True)

            unless(bar_state("the theme the shell runs with").get("theme") == "dark",
                   f"lens --state says theme {bar_state('the theme').get('theme')!r}, expected dark")
            _, output = run("dconf read /org/gnome/desktop/interface/color-scheme", "the colour scheme apps read")
            unless("'prefer-dark'" in without_console(output),
                   f"dconf reads the colour scheme as {without_console(output).strip()!r}, expected 'prefer-dark'")
            _, output = run("systemctl --user show-environment | cat", "the user manager's environment")
            print(f"\nboot-test: the user manager's environment:\n{without_console(output)}", flush=True)
            ...
            unless(not missing, f"the user manager's environment lacks {', '.join(missing)}")
            ...
            def titled_apps(what, png, colors):
                ...
                look(what, png, 120, apps=TITLED_APPS, colors=colors, must=strict, journals=("horizon",))
            ...
            def theme_to(word):
                ...
                run(f"printf '{word}\\n' > ~/.config/rift/theme; and systemctl --user restart lens.service",
                    f"the {word} theme")
                unless(wait_for(30, lambda: bar_state(f"the shell in {word}").get("theme") == word),
                       f"lens --state does not say theme {word} after the shell started again")
                unless(wait_for(30, lambda: scheme in without_console(
                        run("dconf read /org/gnome/desktop/interface/color-scheme", "the colour scheme")[1])),
                       f"the shell did not set the colour scheme to {scheme}")
                part = without_console(run("cat ~/.local/state/rift/horizon.kdl", "horizon's part for the theme")[1])
                unless(f": {word}" in part and ("#f2f1f0" in part) == (word == "light"),
                       f"the part of horizon's config says {part.strip()[-200:]!r} for {word}")
            ...
            theme_to("light")
            point(args.qmp, size, (round(width / 3), round(height / 2)))
            look("the light desktop", f"{stem}-light{extension}", 30, colors=LIGHT_COLORS, must=strict,
            ...
            look("the light Applications menu", f"{stem}-light-menu{extension}", 20, menu=True, rows=listed,
                 colors=LIGHT_COLORS, must=strict, journals=("lens",))
